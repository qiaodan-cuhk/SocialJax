"""
LIO (Learning to Incentivize Others) for SocialJax cleanup environment.
    ...
    def train(rng):
        ...
        def run_rollout(a_params, i_params, c_params, env_st, last_obs, rng):
            ...
            def _env_step(carry, _unused):
                ...
                # (n_envs, n_agents, H, W, C) -> (n_agents, n_envs, H, W, C)
                ...
                actions_stack = jnp.stack(actions)        # (n_agents, n_envs)
                actions_t = actions_stack.T                # (n_envs, n_agents)

                # --- compute incentive rewards ---
                ...
                for k in range(num_agents):
                    ...
                    eta_k = eta_k * mask_k[None, :]        # zero self
                    r_from_others = r_from_others + eta_k.T  # (n_agents, n_envs)
        ...
        def _update_step(runner_state, _unused):
            ...
            for j in range(num_agents):
                obs_j = traj1.obs[:, j]           # (T, n_envs, H, W, C)
                act_j = traj1.action[:, j]         # (T, n_envs)
                ...
                last_obs_j = last_obs[:, j]        # (n_envs, H, W, C)
            ...
            # Pre-flatten trajectory-1 actions for incentive re-computation
            # traj1.action: (T, n_agents, n_envs)
            ...
            for i in range(num_agents):
                ...
                def _meta_loss(eta_i):
                    ...
                    # Agent i's env-reward returns on traj2 (the objective
                    # we want to maximise).  traj2.reward does NOT depend
                    # on eta_i (it is the environment reward collected in
                    # Phase 3 with fixed eta), so stop_gradient for safety.
                    ...
                    for j in range(num_agents):
                        ...
                        # --- re-compute incentive reward for agent j
                        #     as differentiable fn of eta_i ---
                        ...
                        # Compute DISCOUNTED RETURNS for agent j
                        # (not single-step reward).  Reshape incentive
                        # back to (T, n_envs) so compute_returns can
                        # scan over timesteps.  This is differentiable
                        # w.r.t. eta_i through inc_j.
                        inc_j = inc_j_flat.reshape(
                            config["NUM_STEPS"], config["NUM_ENVS"])
                        total_rj = traj1.reward[:, j] + inc_j  # (T, E)
                        returns_j = compute_returns(
                            total_rj, traj1.done[:, j], config["GAMMA"])
                        returns_j_flat = returns_j.reshape(-1)

                        # Baseline for variance reduction.
                        # V(s) comes from the critic which does NOT
                        # depend on eta_i -> stop_gradient.
                        ...
                        # Inner PG gradient, differentiable w.r.t. eta_i.
                        #
                        # adv is passed as an argument (not closed over).
                        # jax.grad differentiates w.r.t. argnums=0
                        # (actor params) only.  Because adv does NOT
                        # depend on actor params, the PG gradient is
                        # correct:  ig = -E[d(log pi)/d(theta) * adv].
                        #
                        # Crucially, adv is NOT stop-gradiented here,
                        # so d(ig)/d(eta_i) = -E[d(log pi)/d(theta)
                        #                        * d(adv)/d(eta_i)]
                        # which gives the non-zero meta-gradient signal.
                        ...
                        # outer loss on traj2 with theta_hat_j
                    ...
                    # regularisation on agent i's own incentive output
            ...
            i_params_new = [s.params for s in i_states]
...
def evaluate(actor_params_list, env, config):
    ...
    root_dir = "evaluation/lio_cleanup"
